...
try {
    ...
    while (rs.next()) {
        ...
        // Remove the comma from the deposit value
        String depositStr = rs.getString("deposit").replace(",", "");
        deposit = Integer.parseInt(depositStr);
        tfpaid.setText(String.valueOf(deposit));
    }

    ResultSet rs2 = c.s.executeQuery("select * from room where roomnumber = '" + tfroom.getText() + "'");
    while (rs2.next()) {
        String price = rs2.getString("price");
        int roomPrice = Integer.parseInt(price.replace(",", ""));
        int amountPaid = roomPrice - deposit;
